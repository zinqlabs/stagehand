...
export const stagehand = base.extend<Fixture>({
  stagePage: async ({ page, context, browser }, use) => {
    if (process.env.BROWSERBASE_API_KEY && process.env.local !== "1") {
      ...
      const bbPage = new StagePage(page, browser, context);
      await use(bbPage);
    } else {
...
stagehand.afterAll(async ({ browser }) => {
...
stagehand.afterEach(async ({ stagePage }) => {
  console.log("test status: ", stagehand.info().status);
  if (stagehand.info().status !== "passed") {
...
stagehand.beforeEach("capture test info", async ({ stagePage }, info) => {
